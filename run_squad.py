...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  if decoder.startswith('MLP'):
    ...
    RecurrentMLPDecoder = rc_decoders_v1.RecurrentMLPDecoder(hidden_size=hidden_size, m_s_l=seq_length)
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    ckpt_step_list = [ckpt_step_list[0]]
    ...
    for item in ckpt_step_list:
      ...
      output_prediction_file = os.path.join(FLAGS.output_dir, str(item)+"_predictions.json")
      output_nbest_file = os.path.join(FLAGS.output_dir, str(item)+"_nbest_predictions.json")
      output_null_log_odds_file = os.path.join(FLAGS.output_dir, str(item)+"_null_odds.json")
